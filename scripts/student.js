...
FriendlyChat.prototype.loadMessages = function() {
  this.messagesRef = this.database.ref('messages').child('users');
  ...
  var setMessage = function(data) {
    ...
    this.displayMessage(data.key, val.username, val.email,val.phone,val.college,val.uid,val.today,val.gradepoint,val.photoUrl, val.imageUrl);
    console.log(val.uid);
  }.bind(this);
...
FriendlyChat.resetMaterialTextfield = function(element1,element2,element3,element4,element5,element6,element7,element8) {
  element1.value = '';
...
function showIt(element) {
  ...
  var cname;
  firebase.database().ref('messages/users/'+firebase.auth().currentUser.uid).once('value').then(function(snapshot) {
       ...
       '<div class="date"></div>' +
        '<div class="phone"></div>' +
         '<div class="audcount"></div>' +
          ...
          '<div class="accept"><button  onclick="showIt(this);" class="acc" >accept</button></div>'+
          ''+
           ' </form>'+
...
FriendlyChat.prototype.displayMessage = function(key, name, email,phone,college,uid,postdate, gradepoint,picUrl, imageUri) {
  ...
  var emailElement = div.querySelector('.email');
  var cElement = div.querySelector('.college');
  var gElement = div.querySelector('.gradepoint');

  if (uid == this.auth.currentUser.uid) { // If the message is text.
    div.querySelector('.email').textContent ="Email : "+ email;
    // Replace all line breaks by <br>.
    phoneElement.textContent =  "Phonenumber : "+phone;
    // Replace all line breaks by <br>.
    phoneElement.innerHTML = phoneElement.innerHTML.replace(/\n/g, '<br>');
    cElement.textContent = "College : "+college;

    // Replace all line breaks by <br>.
    cElement.innerHTML = cElement.innerHTML.replace(/\n/g, '<br>');
    gElement.textContent =  "Grade Point : "+gradepoint;
    // Replace all line breaks by <br>.
    cElement.innerHTML = cElement.innerHTML.replace(/\n/g, '<br>');
